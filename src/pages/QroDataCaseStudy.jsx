...
function NLPDashboardSimulation() {
  ...
  const rawTweets = [
    ...
    { text: "¿Alguien sabe si mañana abren las oficinas?", sentiment: "neu", color: "text-gray-400" },
    { text: "Gran mejora en la actualización de la app.", sentiment: "pos", color: "text-green-400" },
  ];
  ...
  const startEngine = async () => {
    ...
    // Rápido incremento de contador para simular 100k
    ...
    // Flujo visual de tweets pasando por el modelo
    for (let i = 0; i < rawTweets.length; i++) {
      await new Promise(r => setTimeout(r, 600));
      const tweet = rawTweets[i];
      setMentions(prev => [tweet, ...prev].slice(0, 3)); // Mantener max 3 en pantalla
      
      setStats(prev => ({
        ...prev,
        [tweet.sentiment]: prev[tweet.sentiment] + 1
      }));
    }
  ...
  return (
    <div className="w-full bg-[#0a0a0a] rounded-3xl border border-white/10 shadow-2xl overflow-hidden font-sans my-12">
      <div className="grid grid-cols-1 lg:grid-cols-2">
        
        {/* Lado Izquierdo: Data Stream & Engine */}
        <div className="p-6 md:p-8 border-b lg:border-b-0 lg:border-r border-white/10 bg-[#121212] relative">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={startEngine} disabled={isRunning} className="bg-white/10 hover:bg-white/20 disabled:opacity-50 text-white px-4 py-1.5 rounded-lg text-xs font-bold transition-colors flex items-center gap-2">
              ...
              <AnimatePresence>
                {mentions.map((m, i) => (
                  <motion.div 
                    ...
                    className="bg-black/80 backdrop-blur border border-white/10 p-3 rounded-xl flex items-center justify-between"
                  >
                    <span className="text-xs text-white/80 truncate w-[70%] font-mono">"{m.text}"</span>
                    <span className={`text-[10px] font-bold uppercase px-2 py-1 rounded bg-white/5 border border-white/5 ${m.color}`}>
                      {m.sentiment}
                    </span>
                  </motion.div>
                ))}
              </AnimatePresence>
              {!isRunning && mentions.length === 0 && (
                <div className="text-center text-xs text-muted font-mono animate-pulse">Awaiting pipeline execution...</div>
              )}
            </div>
          </div>
        </div>

        {/* Lado Derecho: Dashboard Results */}
        <div className="p-6 md:p-8 flex flex-col justify-center bg-[#050505]">
          ...
          <div className="space-y-4">
            <div>
              <div className="flex justify-between text-xs mb-1"><span className="text-green-400 font-bold">Positive Sentiment</span><span className="text-white">{stats.pos * 14200} mentions</span></div>
              <div className="w-full bg-white/5 rounded-full h-2 overflow-hidden"><motion.div className="bg-green-400 h-full" animate={{ width: `${(stats.pos / 7) * 100 || 0}%` }} transition={{duration:0.5}}/></div>
            </div>
            <div>
              <div className="flex justify-between text-xs mb-1"><span className="text-gray-400 font-bold">Neutral Sentiment</span><span className="text-white">{stats.neu * 14200} mentions</span></div>
              <div className="w-full bg-white/5 rounded-full h-2 overflow-hidden"><motion.div className="bg-gray-400 h-full" animate={{ width: `${(stats.neu / 7) * 100 || 0}%` }} transition={{duration:0.5}}/></div>
            </div>
            <div>
              <div className="flex justify-between text-xs mb-1"><span className="text-red-400 font-bold">Negative Sentiment</span><span className="text-white">{stats.neg * 14200} mentions</span></div>
              <div className="w-full bg-white/5 rounded-full h-2 overflow-hidden"><motion.div className="bg-red-400 h-full" animate={{ width: `${(stats.neg / 7) * 100 || 0}%` }} transition={{duration:0.5}}/></div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-5xl mx-auto px-6">
        ...
        <header className="mb-16 animate-in opacity-0">
          <div className="flex items-center gap-3 mb-6">
            <span className="text-xs font-bold tracking-widest uppercase bg-accent/10 text-accent px-3 py-1 rounded-full border border-accent/20">Data Engineering</span>
          </div>
        ...
        <section className="space-y-20">
          <div className="animate-in opacity-0">
            <h2 className="text-2xl font-bold mb-4 border-b border-white/10 pb-4">01. Live NLP Pipeline Simulation</h2>
          ...
          <div className="animate-in opacity-0 grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            ...
            <div className="rounded-xl overflow-hidden border border-white/10 bg-[#0d1117] shadow-2xl relative">
              <div className="flex items-center px-4 py-3 bg-white/5 border-b border-white/10">
                <span className="text-xs font-mono text-muted">pipeline.py</span>
              </div>
