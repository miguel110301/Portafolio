      ...
      <div className="relative z-10">
          ...
          {nodes.map((node, i) => {
            ...
            return (
              <div key={node.id} className="flex items-center gap-3 md:gap-4">
                <div className={`relative w-24 h-24 md:w-28 md:h-24 rounded-xl border bg-[#0a0a0a] p-3 flex flex-col items-center justify-center transition-all duration-500 ${isActive ? `${node.border} shadow-[0_0_15px_rgba(255,255,255,0.1)] scale-105` : 'border-white/5 opacity-50'}`}>
                  ...
                  {isActive && (
                    <motion.div layoutId="payload-particle" className="absolute -top-2 -right-2 w-6 h-6 bg-accent rounded-full flex items-center justify-center shadow-[0_0_20px_rgba(56,189,248,0.8)] z-20" transition={{ type: "spring", stiffness: 300, damping: 25 }}>
                      <Code2 size={12} className="text-background" />
                    </motion.div>
                  )}
                </div>
                {i < nodes.length - 1 && <ChevronRight size={20} className={`transition-colors duration-500 ${activeNode > i ? 'text-accent' : 'text-white/10'}`} />}
              </div>
            );
          })}
        </div>
        <div className="w-full bg-[#050505] border border-white/10 rounded-xl overflow-hidden shadow-inner">
          <div className="bg-[#1a1a1a] px-4 py-2 flex justify-between items-center border-b border-white/5">
            <span className="text-xs text-muted font-mono flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-green-500 animate-pulse" /> Live Payload Data</span>
            <span className="text-[10px] text-white/30 font-mono">Item 1 of 1</span>
          </div>
          <div className="p-4 h-36 overflow-y-auto">
            <AnimatePresence mode="wait">
              <motion.pre key={activeNode} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ duration: 0.2 }} className="text-xs md:text-sm font-mono text-[#4ade80]">
      ...
      <div className="max-w-5xl mx-auto px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            <h2 className="section-title mb-8">Building the engine <br/>behind the interface.</h2>
            <div className="space-y-4 text-muted text-lg leading-relaxed">
              ...
              <p>
                While my technical foundation lies in backend logic and process orchestration, I also design and implement solutions on the frontend and in native mobile environments.
                This allows me to deliver complete, end-to-end applications where robust server-side power translates into clean, seamless user experiences.
              </p>
            </div>
          </motion.div>
