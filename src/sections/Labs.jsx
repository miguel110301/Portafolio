...
function Labs() {
  ...
  const copy = {
    es: {
      protocol: 'Laboratorio_I_D',
      title: 'Experimentos actuales.',
      description: 'Lo que estoy construyendo ahora mismo. Explorando ecosistemas nativos, probando arquitecturas y resolviendo logica compleja en tiempo real.',
      experiments: [
        {
          title: 'Valance',
          type: 'App Nativa iOS',
          status: 'En desarrollo',
          description: 'Aplicacion de finanzas personales construida de forma nativa para el ecosistema Apple. Actualmente implemento una arquitectura local-first para gastos, componentes de UI personalizados y persistencia estricta.',
          icon: <Smartphone size={24} className="text-purple-400" />,
          tech: ['Swift', 'CoreData', 'Arquitectura iOS'],
        },
        {
          title: 'Step-Tracker RPG',
          type: 'Logica de Juego Movil',
          status: 'Prototipando',
          description: 'Experimento de motor de juego donde los pasos del mundo real se convierten en recursos dentro del juego. Resolviendo retos de podometro en segundo plano, sincronizacion de estado y algoritmos de economia.',
    ...
    en: {
      protocol: 'R&D_Laboratory',
      title: 'Current Experiments.',
      description: 'What I am building right now. Exploring native ecosystems, testing architectures, and solving complex logic in real time.',
      experiments: [
        {
          title: 'Valance',
          type: 'iOS Native App',
          status: 'In Development',
          description: 'Personal finance application built natively for the Apple ecosystem. Currently implementing a local-first architecture for expense tracking, custom UI components, and strict data persistence.',
          icon: <Smartphone size={24} className="text-purple-400" />,
  ...
  return (
    <section id="labs" className="py-32 relative overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto px-6 relative z-10">
        ...
        >
          {copy.experiments.map((exp, index) => (
            <motion.div
              key={index}
